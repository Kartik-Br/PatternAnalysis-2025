...
class DiceCELoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        targets_one_hot = F.one_hot(targets.squeeze(1), num_classes=self.num_classes).permute(0, 4, 1, 2, 3).float()
    ...
    def forward(self, predictions, target):
        ...
        for i, pred in enumerate(predictions):
            ...
            if pred.shape[2:] != target.shape[2:]:
                scaled_target = F.interpolate(target.float(), size=pred.shape[2:], mode='nearest').long()
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lbl_dir', type=str, required=True, help='Directory for training labels.')
    parser.add_argument('--epochs', type=int, default=25, help='Number of training epochs.')
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size.')
    parser.add_argument('--lr', type=float, default=1e-4, help='Learning rate.')
    parser.add_argument('--model_save_path', type=str, default='deep_supervision_unet.pth', help='Path to save the best model.')
    parser.add_argument('--num_classes', type=int, default=6, help='Number of segmentation classes.')
    parser.add_argument('--split_seed', type=int, default=42, help='Random seed for dataset split.')
    ...
    train_idx, val_idx, test_idx = three_way_split_indices(n_items, train_ratio=0.8, val_ratio=0.1, seed=args.split_seed)
    ...
    # Note: test_loader is not used during training, but we save the split for prediction
    ...
    base_loss = DiceCELoss(num_classes=args.num_classes, ce_weights=ce_weights).to(device)
    ...
    print(f"Saved dataset splits to splits.json (80/10/10). Test images: {len(splits['test_image_files'])}")
