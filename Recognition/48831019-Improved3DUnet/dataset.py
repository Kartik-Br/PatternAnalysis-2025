...
class ProstateDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Note: nibabel loads images in (H, W, D), we'll work with (D, H, W) for consistency with PyTorch
        ...
        label_array = zoom(label_array, zoom_factors, order=0, mode='constant', cval=0.0) # Nearest-neighbor for mask
        ...
        sample = {'image': torch.from_numpy(image_array).float(), 'mask': torch.from_numpy(label_array).long()}
